		...
		try {
			if (columns.get("name") != -1) {
			    ...
			    for (int i = firstRowNum + 1; i <= lastRowNum; i++) {
			        row = sh.getRow(i);
			        if (row != null) {
			            cell = row.getCell(columns.get("name"), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK);
			            if (cell.getCellType() == CellType.BLANK) {
			                System.out.println("Vị trí hàng đầu tiên trong cột 'name' có giá trị null: " + i);
			                return i;
			            }
			        }
			    }
			} else {
			    System.out.println("Không tìm thấy cột 'name'.");
			}
